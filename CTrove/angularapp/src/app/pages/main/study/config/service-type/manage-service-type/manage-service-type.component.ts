...
import { ADD_SERVICE_TYPE_SUCCESS, INACTIVATE_ERROR, INVALID_INPUTS, NEW_RECORD, UPDATE_SERVICE_TYPE_SUCCESS } from 'src/Utilities/common/app-strings';
import { SERVICE_TYPE_FIELDS, ServiceType } from 'src/app/models/dto/service-type';
...
@Component({
  ...
  styleUrls: ['./manage-service-type.component.css']
})
export class ManageServiceTypeComponent extends BaseComponent implements OnInit {
  ...
  async submit(isSave: boolean) {
    if(isSave) {
      if(this.form.valid) {
        this.isLoading = true
        let d = new ServiceType().deserialize(this.form.getRawValue())
        ...
        if(this.isAdd) 
          this.service.save(d).subscribe({
            next: (response) => {
              if(!response)
                this.commonService.showErrorMessage()
              this.commonService.showMessage(ADD_SERVICE_TYPE_SUCCESS)
            },
          ...
          this.service.update(d).subscribe({
            next: (response) => {
              if(!response)
                this.commonService.showErrorMessage()
              this.commonService.showMessage(UPDATE_SERVICE_TYPE_SUCCESS)
            },
            error: () => this.commonService.showErrorMessage(),
            complete: () => {
              this.isLoading = false
