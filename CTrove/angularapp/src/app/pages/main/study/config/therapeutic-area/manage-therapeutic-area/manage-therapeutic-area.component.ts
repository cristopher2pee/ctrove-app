...
import { ADD_TA_SUCCESS, INACTIVATE_ERROR, INVALID_INPUTS, NEW_RECORD, UPDATE_TA_SUCCESS } from 'src/Utilities/common/app-strings';
import { THERAPEUTIC_AREA_FIELDS, TherapeuticArea } from 'src/app/models/dto/therapeutic-area';
...
@Component({
  selector: 'app-manage-therapeutic-area',
  templateUrl: './manage-therapeutic-area.component.html',
  styleUrls: ['./manage-therapeutic-area.component.css']
})
export class ManageTherapeuticAreaComponent extends BaseComponent implements OnInit {
  ...
  async submit(isSave: boolean) {
    if(isSave) {
      if(this.form.valid) {
        this.isLoading = true
        let d = new TherapeuticArea().deserialize(this.form.getRawValue())
        ...
        if(this.isAdd) 
          this.service.save(d).subscribe({
            next: (response) => {
              if(!response)
                this.commonService.showErrorMessage()
              this.commonService.showMessage(ADD_TA_SUCCESS)
            },
          ...
          this.service.update(d).subscribe({
            next: (response) => {
              if(!response)
                this.commonService.showErrorMessage()
              this.commonService.showMessage(UPDATE_TA_SUCCESS)
            },
            error: () => this.commonService.showErrorMessage(),
            complete: () => {
              this.isLoading = false
