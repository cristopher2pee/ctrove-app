...
import { ADD_TC_SUCCESS, INACTIVATE_ERROR, INVALID_INPUTS, NEW_RECORD, UPDATE_TC_SUCCESS } from 'src/Utilities/common/app-strings';
import { TRIAL_CLASSIFICATION_FIELD, TrialClassification } from 'src/app/models/dto/trial-classification';
...
@Component({
  selector: 'app-manage-trial-classification',
  templateUrl: './manage-trial-classification.component.html',
  styleUrls: ['./manage-trial-classification.component.css']
})
export class ManageTrialClassificationComponent extends BaseComponent implements OnInit {
  isAdd: boolean = true
  data?: TrialClassification | null
  ...
  async submit(isSave: boolean) {
    if(isSave) {
      if(this.form.valid) {
        this.isLoading = true
        let d = new TrialClassification().deserialize(this.form.getRawValue())
        ...
        if(this.isAdd) 
          this.service.save(d).subscribe({
            next: (response) => {
              if(!response)
                this.commonService.showErrorMessage()
              this.commonService.showMessage(ADD_TC_SUCCESS)
            },
          ...
          this.service.update(d).subscribe({
            next: (response) => {
              if(!response)
                this.commonService.showErrorMessage()
              this.commonService.showMessage(UPDATE_TC_SUCCESS)
            },
            error: () => this.commonService.showErrorMessage(),
            complete: () => {
              this.isLoading = false
